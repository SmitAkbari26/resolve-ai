...
export const Sidebar: React.FC = () => {
  ...
  const links = NAV_BY_ROLE[role] ?? NAV_BY_ROLE.customer;
  ...
  return (
    <aside className="w-64 bg-[#090d1f]/90 text-white flex flex-col h-screen select-none border-r border-slate-800/80 shadow-2xl backdrop-blur-xl">
            ...
            >
              <Icon
                className={`w-4 h-4 transition-transform duration-200 ${
                  isActive ? "text-indigo-400" : "text-slate-500 group-hover:scale-105"
                }`}
              />
              <span>{link.name}</span>
            </button>
      ...
      {/* Footer */}
      <div className="p-4 border-t border-slate-800/60 bg-[#060a17]/50 space-y-3">
        {/* User info */}
        <div className="px-2 space-y-0.5">
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">
            Signed in as
          </span>
          <span className="text-xs text-slate-300 truncate block">{user?.email ?? "—"}</span>
          <span className={`text-[10px] font-bold uppercase tracking-wider ${ROLE_COLOURS[role]}`}>
            {role}
          </span>
        </div>
        ...
        <button
          onClick={logout}
          className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 rounded-xl text-xs font-semibold border border-rose-500/20 transition-all shadow-sm"
        >
