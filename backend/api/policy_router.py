...
async def create_policy_api(payload: PolicyCreate, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_policies_api(db: AsyncSession = Depends(get_db)):
    service = PolicyService(db)
...
async def get_active_policies_api(db: AsyncSession = Depends(get_db)):
    service = PolicyService(db)
    return await service.get_active_policies()


@router.get("/{policy_id}", response_model=PolicyResponse)
async def get_policy_api(policy_id: UUID, db: AsyncSession = Depends(get_db)):
    service = PolicyService(db)
...
async def delete_policy_api(policy_id: UUID, db: AsyncSession = Depends(get_db)):
    service = PolicyService(db)
